...
test("should set the color to the provided colorHex", () => {
  const config = { color: 0xff0000 };
  const params = new VectorPlotConfig(config);
  expect(params.color).toBe(0xff0000);
});

test("should set the color to default it undefined passed", () => {
  const config = {};
  const params = new VectorPlotConfig(config);
  expect(params.color).toBe(0x000000);
});

test("should create a new VectorPlotPlaneConfigParams object for xy, xz, yz when enabled singolarly", () => {
...
test("should keep xy, xz, yz as undefined if passed as undefined", () => {
...
test("should set xy, xz, and yz to undefined if they are not provided", () => {
  const config = {};
  const params = new VectorPlotConfig(config);
  expect(params.xy).toBeUndefined();
  expect(params.xz).toBeUndefined();
...
test("should keep the angle undefined if no angle is provided", () => {
  const config = {};
  const params = new VectorPlotConfig(config);
  expect(params.angle).toBeUndefined();
});

test("should keep the angle undefined if config undefined", () => {
  const config = undefined;
  const params = new VectorPlotConfig(config);
  expect(params.angle).toBeUndefined();
});

test("should set the angle to the provided angle object if it exists", () => {
...
test("should initialize all optional components to their default when true is passed as argument", () => {
...
test("should initialize all properties to their default when an object with all properties set to true is passed as argument", () => {
...
test("should initialize projection component to its default when projection is true", () => {
...
test("should initialize component component to its default when component is true", () => {
...
test("should initialize projectionAngle compoenent to its default when projectionAngle is true", () => {
...
test("should initialize all properties to undefined when an empty object is passed as argument", () => {
  const configParams = new VectorPlotPlaneConfig({});
...
test("should initialize all properties to undefined when an object with all components set to false is passed as argument", () => {
  const configParams = new VectorPlotPlaneConfig({
    projection: false,
    component: false,
...
test("should initialize integrate a partial configuration defining only the label with the line options", () => {
...
test("for a single component, should initialize a default line if only the label is defined ", () => {
  const partialConfig = { label: { text: "s" } };
